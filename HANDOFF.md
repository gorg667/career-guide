...
- 2026-09-04: Interruption #4 hit exactly on the PR-creation call (account switch). Sandbox survived this time; verified via API that no PR existed, then created **PR #4: https://github.com/gorg667/career-guide/pull/4**. Gotcha for future instances: the GitHub token in `~/.git-credentials` may have prefix `ghu_` (not only `ghp_`/`gho_`) — parse it with `urllib.parse.urlparse(line).password`, don't grep by prefix. Phase 4 is complete; nothing is pending except the owner enabling Pages and merging PR #4.
